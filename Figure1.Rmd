...
# Figure 1d
```{r}
df1d <- read.delim("/Users/chey2/Desktop/DOCK8_paper_codes/Input_files", header = TRUE, sep = "\t", colClasses = "numeric") 

p1d <- ggplot(df1d, aes(x=Samples)) +
...
# Figure 1e
```{r}
df1e <- read.delim("/Users/chey2/Desktop/DOCK8_paper_codes/Input_files/Fig1e_input.txt", header = TRUE, sep = "\t", colClasses = c("factor", "factor", "factor", "numeric")) # input
...
# Figure 1f
```{r}
df1f <- read.delim("/Users/chey2/Desktop/DOCK8_paper_codes/Input_files/Fig1f_input.txt", header = TRUE, sep = "\t", colClasses = c("factor", "factor", "factor", "numeric")) # input
...
# Figure 1h
```{r}
df1h1i <- read.delim("/Users/chey2/Desktop/DOCK8_paper_codes/Input_files/Fig1h_input.txt", header = TRUE, sep = "\t", colClasses = c("character", "factor", "factor", "factor", "factor", "numeric", "numeric")) # input

df1h1i$site_specific <- factor(df1h1i$site_specific, levels = c("Ra", "Mb", "Vf", "Hp", "Ac", "Ic", "Pc", "Ph"))
df1h1i$subject_id <- factor(df1h1i$subject_id, levels = c("Pt01", "Pt02", "Pt03", "Pt04", "Pt05", "Pt06", "Pt07", "Pt08", "Pt09", "Pt10", "Pt11", "Pt12", "Pt13", "Pt14", "Pt15", "Pt16", "Pt17", "Pt18", "Pt19", "Pt20", "Pt21", "Pt22", "Pt23", "Pt24", "Hv01", "Hv02", "Hv03", "Hv04", "Hv05", "Hv06", "DPt07", "DPt08", "DPt09", "DPt10", "DPt14", "DPt15", "DPt18"))
df1h1i$Group <- factor(df1h1i$Group, levels = c("Pre-Tx", "Post-Tx-onIS-1M", "Post-Tx-onIS-2M", "Post-Tx-onIS-3M","Post-Tx-onIS-6M","Post-Tx-onIS>12M","Post-Tx-offIS-12M","Post-Tx-offIS-24M","HVs", "Donor"))
...
p1h <- cowplot::plot_grid(plotlist = lapply(p_pre_postoff12_highlight_3_sites, "[[", 1), ncol = 1)
p1h
```

# Figure 1i 
```{r}
df1i <- read.delim("/Users/chey2/Desktop/DOCK8_paper_codes/Input_files/Fig1i_input.txt", header = TRUE, sep = "\t", colClasses = c("factor", "factor", "factor", "numeric", "numeric")) # input

df1i$group <- factor(df1i$group, levels = c("Pre-Tx", "Post-Tx-onIS-1M", "Post-Tx-onIS-2M", "Post-Tx-onIS-3M", "Post-Tx-onIS-6M", "Post-Tx-offIS-12M", "Post-Tx-offIS-24M"))
df1i$kingdom <- factor(df1i$kingdom, levels = c("Bacteria", "Fungi", "Eukaryotic virus"))
df1i$site_specific <- factor(df1i$site_specific, levels = c("Ra", "Mb", "Vf", "Hp", "Ac", "Ic", "Pc", "Ph"))
df1i$sd[is.na(df1i$sd)] = 0
